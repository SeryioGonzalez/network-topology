...
spokeType="nat_spoke_vnets"
templateFile=$natSpokeVnetTemplateFile
...
do
	...
	spoke_FirewallSubnet_cidr=$(jq -r ".$spokeType[$spokeIndex].FirewallSubnet_cidr"  $spokeDataFile)
	...
	az group deployment create --no-wait --resource-group $resGroupName --template-file "$templateFile" \
		...
		--parameters spoke_FirewallSubnet_cidr="$spoke_FirewallSubnet_cidr" 
	set +x	
done
